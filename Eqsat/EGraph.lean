...
open TRS TreeAutomaton in section
...
theorem automaton_deterministic'
    {pcr : PCR S} {t : Term S} {q₁ q₂ : pcr.Classes} (s₁ : Steps pcr.automaton.trs t q₁)
    (s₂ : Steps pcr.automaton.trs t q₂) : q₁ = q₂ := by
  let fn ° as := t
  have ⟨qs₁, mem₁, ⟨s₁⟩⟩ := Accepts.final ⟨s₁⟩
  have ⟨qs₂, mem₂, ⟨s₂⟩⟩ := Accepts.final ⟨s₂⟩
  have h := funext fun i =>
    have ⟨s₁⟩ := steps_child ⟨s₁⟩ i
    have ⟨s₂⟩ := steps_child ⟨s₂⟩ i
    automaton_deterministic' s₁ s₂
...
theorem automaton_deterministic
    ...
    q₁ = q₂ :=
  hq₁.elim fun s₁ => hq₂.elim fun s₂ => automaton_deterministic' s₁ s₂

theorem automaton_reachable (pcr : PCR S) : pcr.automaton.Reachable := by
  refine Quotient.ind fun ⟨t, h⟩ => ⟨t, ?_⟩
  induction t
  case app ih =>
    exact .tail (Steps.children fun i => ih i <| pcr.reach h i) (step_of_transition <| .intro ..)
